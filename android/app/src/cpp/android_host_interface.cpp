...
std::FILE* AndroidHostInterface::OpenFile(const char* path, const char* mode)
{
  ...
  {
    return CommonHostInterface::OpenFile(path, mode);
  }
                       ...
                       jlong modification_time)
{
  ...
  return gl->ScanFile(std::move(filename_str));
}
